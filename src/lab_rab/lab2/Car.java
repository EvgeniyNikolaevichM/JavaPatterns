...
public class Car implements IVehicle, Serializable, Cloneable{
    ...
    //Здесь выставляется тот поток куда будет записываться команда
...
// Iterator
    //Поведенческий паттерн, который позволяет последовательно обходить, ну обычно для
    //коллекции он используется, он реализует 3 основных метода:hasNext next remove
    //Запускаем в мейн
    public AutoIterator iterator() {
        return new AutoIterator();
    }

    class AutoIterator implements java.util.Iterator {//Стандартный интефейс наследуем
        ...
        }//Проверяет есть ли элемент след в массиве
...
//Memento - поведенческий паттерн, который позволяет сохранить текущее
//состояние объекта
    public static class Memento implements Serializable {

        private static final ByteArrayOutputStream state = new ByteArrayOutputStream(64);
//Здесь состояние сохраняется:
...
//Здесь состояние получаем:
        public static Car getAuto() throws IOException, ClassNotFoundException {
            ...
            return (Car) objectInputStream.readObject();//получаем состояние

        }
    }

// 2 внешних метода, которые работают с внутренними:
