...
//Вызываем фабричный метод и распечатываем каждый раз класс созданного объекта, чтобы показать кого создаем
...
//Паттерн прототип: Клонирование в предыдущих(искать кар и мото clone)
//Реализует клонирование(помогает создать копию объекта) КЛОНИРОВАНИЕ ГЛУБОКОЕ
//клонируется все:  (при обычном создается объект и клонируются ссылки)
/*        Motorcycle moto1 = new Motorcycle("Moto", 2);
        Motorcycle cloned = moto1.clone();
        moto1.setModelName("Moto1", "Model new");
...
/*        //Запись ведется в файлы RowChain.txt и ColumnChain.txt
        ...
        columnChain.setNextChain(rowChain);//устанавливаем, что для чины колом чейн след чина роу чейн
...
/*
         Car car = new Car("iterator", 3);//Создали машину
        Car.AutoIterator iterator = car.iterator();//Создали итератор
        while (iterator.hasNext()) {
            System.out.println(iterator.next());//Вывели
        }
*/
...
/*        Car car5 = new Car("Memento", 3);
        Vehicles.printVehicle(car5);//Вывод до изменения состояния
        car5.createMemento();
        car5.addModel("New", 500);//Здесь изменение состояния
        Vehicles.printVehicle(car5);//Вывод после изменения состояния
